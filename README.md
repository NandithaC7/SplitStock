...
```
backend/               Django + DRF + Channels + Celery + PostgreSQL
frontend/              React (Vite) + D3 + Framer Motion + Zustand
```
...
```bash
cd backend && USE_INMEMORY_CHANNEL_LAYER=1 python manage.py test
